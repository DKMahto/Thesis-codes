...
def rename_techs(label):
    ...
    rename = {
        ...
        "mdes-charger": "Medium-duration Energy Storage Charger",
        "mdes-discharger": "Medium-duration Energy Storage Discharger",
        "mdes-inverter": "Medium-duration Energy Storage Inverter",
    }
...
def plot_costs():
    ...
    # new_columns = df.sum().sort_values().index (updated)
    ...
    # update - df.loc[new_index, new_columns].T.plot()
...
# updated
    ...
    # update - new_columns = df.columns.sort_values()

    fig, ax = plt.subplots(figsize=(12, 8))

    logger.debug(df.loc[new_index]) # logger.debug(df.loc[new_index, new_columns])
...
# deleted something recorded in `add_storage_technologies.py` -> it works
def historical_emissions(countries):
    ...
    df = pd.read_csv(snakemake.input.co2, encoding="latin-1", low_memory=False) # input co2 but in bundle-sector
    ...
    e["indirect"] = "ind_CO2 - Indirect CO2" # e["total wL"] = "Total (with LULUCF)"
    ...
    # updated - year = np.arange(1990, 2018).tolist()
...
def plot_carbon_budget_distribution(input_eurostat):
    ...
    if snakemake.config["foresight"] == "myopic":
        path_cb = "results/" + snakemake.params.RDIR + "/csvs/" # this is deleted in the updated version
    ...
    # updated - deleted `ax1.plot(e_1990 * CO2_CAP[o], linewidth=3, color="dodgerblue", label=None)`
    ...
    # ax1.plot([2050], [0.01 * emissions[1990]], marker="*", markersize=12, markerfacecolor="white", linewidth=0,markeredgecolor="black",
    ...
    ax1.plot(
        [2050],
        [0.0 * emissions[1990]], # [0.125 * emissions[1990]]
        # "ro",
        marker="*",
        markersize=12,
        markerfacecolor="black",
    ...
    plt.grid(axis="y")
    # original - path_cb_plot = "results/" + snakemake.params.RDIR + "/graphs/"
    path = snakemake.output.balances.split("balances")[0] + "carbon_budget.pdf"
    plt.savefig(path + "carbon_budget_plot.pdf", dpi=300) # (path_cb_plot + "carbon_budget_plot.pdf", dpi=300)
...
if __name__ == "__main__":
    ...
    # for sector_opts in snakemake.params.sector_opts:
        ...
        # for o in opts:
            # if any("cb" in o for o in opts) or snakemake.config["foresight"] == "perfect": # cb" in o:
